...
# Set the secret key for the Flask application (for session management and CSRF protection)
...
# Define the register route with both GET and POST methods
...
def register():
    ...
    # Render the register template for GET requests
    return render_template('register.html')
...
def overview():
    ...
    if 'logged_in' in session:
        ...
        # Depending on the user role, fetch API pulls data
        if session['role'] == 1:
        ...
        # If API pulls data is available, process it
        if len(api_pulls) != 0:
            # Convert the fetched data into a DataFrame
            df = pd.DataFrame(api_pulls, columns=['id', 'created', 'key', 'origin'])
            df['created'] = pd.to_datetime(df['created'])

            # Generate charts and get top origins
            ...
            # If no API pulls data, prepare to render with placeholders
            df = pd.DataFrame(api_pulls, columns=['id', 'created', 'key', 'origin'])
            df['created'] = pd.to_datetime(df['created'])

            top_origins = get_top_origins(df)
            # Return template with placeholder data for charts
...
def index():
    ...
    # Render template with articles data
...
def article(id):
    ...
    # Render template with the article data
...
def edit(id):
    ...
    if 'logged_in' in session:
        ...
        # Render edit.html template with article data for GET requests
...
@app.route('/scores')
def scores():
    # Render scores.html template
    return render_template('scores.html')


@app.route('/privacy-statement')
def privacy():
    # Render privacy.html template
    return render_template('privacy.html')


@app.route('/developers')
def developers():
    # Render developers.html template
    return render_template('developers.html')


@app.route('/developers/creator')
def iframe_creator():
    # Render iframe_creator.html template with specific parameters
...
def api_keys():
    # Check if the user is logged in
    if 'logged_in' in session:
        # Retrieve API keys based on user role
        conn = get_db_connection()
        if session['role'] == 1:
    ...
    if 'logged_in' in session:
        ...
        if request.method == 'POST':
            ...
            if not note:
                ...
                # Generate a hashed API key based on note, current timestamp, and app secret key
                ...
                # Redirect to the API keys page after successful insertion
                flash('Your API key has been created!')
                return redirect(url_for('api_keys'))

        # Render the create_api_key.html template for GET requests
...
def edit_api_key(id):
    # Check if the user is logged in
    if 'logged_in' in session:
        # Retrieve the API key to edit
        db_api_key = get_api_key(id)

        # Handle POST request to update API key note
        if request.method == 'POST':
            ...
            if not note:
                ...
                # Update API key note in the database
                conn = get_db_connection()
                conn.execute(
        ...
        # Render edit_api_key.html template with API key data for GET requests
...
def delete_api_key(id):
    ...
    if 'logged_in' in session:
        ...
        # Flash message confirming deletion and redirect to api_keys page
...
def api(key):
    ...
    if db_api_key:
        ...
        # Dictionary to store scores by car brand
        car_brand_scores = {}

        # Convert articles to list of dictionaries
        articles_list = [dict(article) for article in articles]

        # Populate car_brand_scores with scores from articles
        ...
        # Calculate mean scores for each car brand and category
        ...
        if 'application/xml' in request.headers.get('Accept', ''):
            # Convert car_brand_scores to XML and return as response
            ...
            # Return car_brand_scores as JSON if no preference or prefers JSON
            return jsonify(car_brand_scores)

    # Return 400 error if API key not found
    return Response(
...
def integration(company=None, key=None):
    # Default values for styling parameters
    ...
    # Handle requests with company and key parameters
    if company and key:
        # Retrieve API key from database
        db_api_key = get_api_key(None, key)

        # Proceed if API key exists
        if db_api_key:
            key_id = db_api_key['id']
            ...
            try:
                with get_db_connection() as conn:
                    ...
                    # Retrieve articles for the specified company
                    articles = conn.execute('SELECT * FROM articles WHERE LOWER(company) = ?',
                                            (company.lower(),)).fetchall()

                    if articles:
                        # Dictionary to store scores for the specified company
                        car_brand_score = {}
                        articles_list = [dict(article) for article in articles]

                        # Calculate mean scores for customer service, reliability, and responsibility
                        for article in articles_list:
                            car_brand = article.get('company')
                        ...
                        # Render iframe.html with scores and styling parameters
                    ...
                    # Render iframe.html with company name and styling parameters
                    return render_template('iframe.html',
                                           company=company,
                                           color_bg=color_bg,
                                           color_container=color_container,
    ...
    # Render iframe.html with default values for styling parameters
    return render_template('iframe.html',
...
def contact():
    if request.method == 'POST':
        ...
        if not name or not subject or not email or not message:
            ...
            flash('Invalid email address!')
            return render_template('contact.html', name=name, subject=subject, email=email, message=message)
        elif not re.match(r'[A-Za-z0-9]+', name):  # Validate name format (alphanumeric)
            flash('Invalid name!')
            return render_template('contact.html', name=name, subject=subject, email=email, message=message)
        elif not re.match(r'[A-Za-z0-9]+', subject):  # Validate subject format (alphanumeric)
            ...
            sender_email = os.getenv('SMTP_USER')  # Your Gmail address
            password = os.getenv('SMTP_PASS')  # Your Gmail password
            receiver_email = email  # Email address to which the contact form message will be sent
...
{message}"""
            ...
            # Attach the formatted message as plain text
            ...
            try:
                ...
                # Flash a success message and clear the form fields
    ...
    # Render the contact form for GET requests or if there are validation errors
    return render_template('contact.html', name='', subject='', email='', message='')
...
# Function to fetch an article from the database by its ID
def get_article(id):
    conn = get_db_connection()
    article = conn.execute('SELECT * FROM articles WHERE id = ?', (id,)).fetchone()
    conn.close()
    if article is None:
        flash('Invalid Article ID!')  # Flash message if no article found
        return redirect(url_for('index'))  # Redirect to the index page
    return article  # Return the fetched article


# Function to retrieve an API key either by ID or key value
def get_api_key(id=None, key=None):
    conn = get_db_connection()
    api_key = ''
    if id is not None:
        api_key = conn.execute('SELECT * FROM api_keys WHERE id = ?', (id,)).fetchone()
        conn.close()
        if api_key is None:
            flash('Invalid API key!')  # Flash message if no API key found
            return redirect(url_for('api_keys'))  # Redirect to the API keys page
    elif key is not None:
        api_key = conn.execute('SELECT * FROM api_keys WHERE key = ?', (key,)).fetchone()
        conn.close()
    return api_key  # Return the fetched API key
